...
void countDirectorySizes(fileNode* node, int& total)
{
	for (auto file : node->children) {
		if (file->dir) {
			if (file->size <= 100000)
				total += file->size;
			countDirectorySizes(file, total);
		}
	}
...
int main(int argc, char* argv[])
{
	...
	int total = 0;
	countDirectorySizes(root, total);
	std::cout << "sum amount dirs over 100000: " << total << std::endl;
	delete root;
	return 0;
}